...
therml = ax.imshow(np.zeros((24, 32)), vmin=0, vmax=60)
cbar = fig.colorbar(therml)
...
while True:
    ...
    while retry_count < max_retries:
        try:
            ...
            therml.set_data(np.fliplr(data_array))
            therml.set_clim(vmin=np.min(data_array), vmax=np.max(data_array))
